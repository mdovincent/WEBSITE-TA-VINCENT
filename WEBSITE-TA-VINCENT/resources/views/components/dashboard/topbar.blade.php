@php
    ...
    $topbarLinks = [
        ['label' => 'Keranjang', 'count' => 2, 'icon' => 'cart', 'hash' => 'keranjang'],
        ['label' => 'Pesan', 'count' => 1, 'icon' => 'chat', 'hash' => 'pesan'],
        ['label' => 'Notifikasi', 'count' => 2, 'icon' => 'bell', 'hash' => 'notifikasi'],
    ];
@endphp
    ...
    <div class="flex h-full items-center gap-3 px-4 sm:px-5 lg:px-6">
        ...
        <div class="ml-auto flex shrink-0 items-center gap-0.5 sm:gap-1.5">
            @foreach ($topbarLinks as $action)
                <a href="{{ $dash($action['hash']) }}" class="relative flex h-12 w-12 items-center justify-center rounded-xl text-gray-600 transition hover:bg-gray-100 hover:text-umkm-brown" aria-label="{{ $action['label'] }}">
                    @if ($action['icon'] === 'cart')
